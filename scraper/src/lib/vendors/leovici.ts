  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        'body > table > tbody > tr > td > table:nth-child(3) > tbody > tr > td > center > table:nth-child(2) > tbody > tr > td > table:nth-child(1) > tbody'
      );
      ...
      initialContainer.querySelectorAll('tr').forEach((item) => {
        ...
        const isProduct = !!hasTumbnail && item.children.length === 3 && item.childNodes.length === 6;
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement = productRowElement.querySelector('td:nth-child(2) > span:nth-child(1)');
        const productName = productNameElement.textContent.slice(0, -3).trim() || /* istanbul ignore next */ '';

        const productQuantityElement = productRowElement.querySelector('td:nth-child(2) > span:nth-child(1)');
        const productQuantity = productQuantityElement.textContent.slice(-1).trim();
