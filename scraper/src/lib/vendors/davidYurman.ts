...
export default class DavidYurman {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table:nth-child(4) > tbody > tr > td:nth-child(1) > p:nth-child(2)'
      );
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderDateElement = root.querySelector(
        'body > table:nth-child(4) > tbody > tr > td:nth-child(2) > p:nth-child(2)'
      );
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector('body > table:nth-child(6) > tbody');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productQuantityElement = productRowElement.querySelector('td:nth-child(2) > p:nth-child(5)');
        const productQuantity = productQuantityElement.childNodes[1].textContent.trim();
        ...
        const productPriceElement = productRowElement.querySelector('td:nth-child(2) > p:nth-child(6)');
        const price = /* istanbul ignore next */ productPriceElement.childNodes[1].textContent.replace('$', '').trim();
