...
export default class JJill {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table > tbody > tr > td > table:nth-child(6) > tbody > tr > td:nth-child(2)'
      );
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderDateElement = root.querySelector(
        'body > table > tbody > tr > td > table:nth-child(5) > tbody > tr > td:nth-child(2)'
      );
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector('body > table > tbody > tr > td');

      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item: any) => {
        const hasTumbnail = item.querySelector('td:nth-child(1) > a > img');
        const isProduct = !!hasTumbnail && item.children.length === 5 && item.childNodes.length === 11;
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement = productRowElement.querySelector('td:nth-child(3) > a > strong > span');
        const productName = productNameElement.textContent.trim() || /* istanbul ignore next */ '';

        // no qty displayed in template, returning 1
