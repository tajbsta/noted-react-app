...
export default class Aveda {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table:nth-child(1) > tbody > tr:nth-child(3) > td > div > div > p:nth-child(3)'
      );

      const orderRef = orderRefElement
        ? orderRefElement.childNodes[0].textContent.trim().split(' and ')[0].substr(-10)
        : null;
      return `${orderRef}`;
    } catch (error) {
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector('#product-info > tbody');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement = productRowElement.querySelector('td:nth-child(2) > h4');
        const productName = productNameElement.textContent.replace(/\s+/g, ' ').trim() || '';
        ...
        const productPriceElement = productRowElement.querySelector('td:nth-child(4) > div');
