  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'html > body > table > tbody > tr > td > table:nth-child(6) > tbody > tr > td > font'
      );

      const orderRef = orderRefElement
        ? orderRefElement.textContent.split('#').pop().split('And').shift().trim()
        : null;
      return `${orderRef}`;
    } catch (error) {
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderDateElement = root.querySelector(
        'html > body > table > tbody > tr > td > table:nth-child(9) > tbody > tr > td:nth-child(2) > font > strong'
      );
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        'html > body > table > tbody > tr > td > table:nth-child(25) > tbody > tr > td'
      );
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productPriceElement = productRowElement.querySelector('td:nth-child(7) > font > span');
