  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        '#template-root > tbody > tr:nth-child(2) > td > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > p'
      );
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        '#template-root > tbody > tr:nth-child(2) > td > table:nth-child(7) > tbody > tr > td > table > tbody'
      );

      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item) => {
        const hasTumbnail = item.querySelector('td:nth-child(1) > table > tbody > tr > td > a > img');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productQuantityElement = productRowElement.querySelector('td:nth-child(2) > p:nth-child(1)');
