...
export default class TheContainerStore {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table > tbody > tr > td > center > table:nth-child(2) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td.callout-wpr.center.outlook-padding-top > p.callout.center > b'
      );
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        'body > table > tbody > tr > td > center > table:nth-child(5) > tbody > tr > td > table > tbody > tr > td > table > tbody > tr > td.center > center > table:nth-child(1) > tbody'
      );

      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item) => {
        const hasPrice = item.querySelector('td:nth-child(3)')?.textContent.includes('$');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productQuantityElement = productRowElement.querySelector('td:nth-child(1)');
