...
export default class JohnnyWas {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector('body > table > tbody > tr:nth-child(8) > td');
      const orderRef = orderRefElement
        ? orderRefElement.childNodes[0].textContent.replace('ORDER NUMBER:', '').trim()
        : null;
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderDateElement = root.querySelector('body > table > tbody > tr:nth-child(8) > td');

      const orderDate = orderDateElement
        ? orderDateElement.childNodes[2].textContent.replace('ORDER DATE:', '').trim()
        : null;
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector('body > table > tbody > tr:nth-child(9) > td > table');

      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item: any) => {
        const hasTumbnail = item.querySelector('td:nth-child(1) > p > img');
        const isProduct = !!hasTumbnail && item.children.length === 4 && item.childNodes.length === 4;
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement = productRowElement.querySelector('td:nth-child(1) > p');
