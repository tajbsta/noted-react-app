...
export default class Evereve {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table:nth-child(2) > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(3) > td > span'
      );
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderRefElement = root.querySelector(
        'body > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table:nth-child(2) > tbody > tr > td:nth-child(1) > table > tbody > tr:nth-child(3) > td > span > span'
      );
      const orderDate = orderRefElement ? orderRefElement.textContent.trim() : null;

      return orderDate ? moment(orderDate, 'MMM DD, YYYY, hh:mm:ss A').startOf('day').valueOf() : null;
    } catch (error) {
      /* istanbul ignore next */
      return null;
    }
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        'body > table > tbody > tr > td > table > tbody > tr:nth-child(2) > td > table.email-items'
      );
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productPriceElement = productRowElement.querySelector('td:nth-child(4) > span');
