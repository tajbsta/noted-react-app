  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector(
        'html > body > table > tbody > tr:nth-child(4) > td > table > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(5) > td > table > tbody > tr > td:nth-child(1)'
      );
  ...
  static async getOrderDate(root: Document): Promise<number | null> {
    try {
      const orderDateElement = root.querySelector(
        'html > body > table > tbody > tr:nth-child(4) > td > table > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(5) > td > table > tbody > tr > td:nth-child(2)'
      );
      const orderDate = orderDateElement ? orderDateElement.textContent.split(':').pop()?.trim() : null;

      return orderDate ? moment(orderDate, 'MMM DDD, YYYY').startOf('day').valueOf() : null;
    } catch (error) {
      /* istanbul ignore next */
      return null;
    }
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector(
        'html > body > table > tbody > tr:nth-child(4) > td > table > tbody > tr > td:nth-child(2) > table > tbody > tr:nth-child(8) > td > table > tbody > tr:nth-child(1) > td:nth-child(1) > table > tbody'
      );
      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item) => {
        const hasThumbnail = item.querySelector('td:nth-child(1) > img')?.getAttribute('src');
        const hasProductName = item.querySelector('td:nth-child(2) > strong');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const mainText = productNameElement.querySelector('strong').textContent;

        const productName =
          `${mainText} - ${productNameElement.innerHTML.split('<br>')[1].trim()}` || /* istanbul ignore next */ '';
        const productQuantityElement = productRowElement.querySelector(
          'td:nth-child(2) > table > tbody > tr:nth-child(3) > td:nth-child(2)'
        );
        ...
        const productPriceElement = productRowElement.querySelector('td:nth-child(3) > strong');
