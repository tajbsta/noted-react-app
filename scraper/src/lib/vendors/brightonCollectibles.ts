...
export default class BrightonCollectibles {
  ...
  static async getOrderRef(root: Document): Promise<string | null> {
    try {
      const orderRefElement = root.querySelector('body > table:nth-child(3) > tbody > tr > td:nth-child(2) > b');
  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      const initialContainer = root.querySelector('body > table:nth-child(4) > tbody');

      const orderTableContainer = [];

      initialContainer.querySelectorAll('tr').forEach((item) => {
        const hasThumbnail = item.querySelector('td:nth-child(2) > img');
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement = productRowElement.querySelector('td:nth-child(3) > b > a');
        const productName = productNameElement.textContent.trim() || '';

        const productQuantityElement = productRowElement.querySelector('td:nth-child(3)');
        const productQuantity = productQuantityElement.childNodes[5].textContent.replace('Quantity: ', '').trim() || '';
