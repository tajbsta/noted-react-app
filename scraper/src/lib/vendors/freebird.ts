  ...
  static async getProducts(root: Document): Promise<RawProduct[]> {
    try {
      ...
      orderTableContainer.forEach((item) => {
        ...
        const productNameElement1 = productRowElement.querySelector('td:nth-child(2) > span:nth-child(1)');
        const productName1 =
          productNameElement1.textContent
            .replace(/(?!\w|\s)./g, '')
            .slice(0, -3)
            .trim() || '';
        const productNameElement2 = productRowElement.querySelector('td:nth-child(2) > span:nth-child(3)');
        const productName2 = productNameElement2.textContent.split('/')[0].trim();
