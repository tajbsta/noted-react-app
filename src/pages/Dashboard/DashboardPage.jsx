...
import { getUserId, getUser, updateUserAttributes, scrapeOlderEmails } from '../../api/auth';
import { getOrders } from '../../api/orderApi';
import { getAccounts } from '../../api/accountsApi';
import { clearSearchQuery } from '../../actions/runtime.action';
import { setCartItems } from '../../actions/cart.action';
import { LAST_CALL, NOT_ELIGIBLE, RETURNABLE, DONATE } from '../../constants/actions/runtime';
...
export default function DashboardPage() {
	...
	async function fetchAccounts(userId) {
		try {
			...
			const scrapeNotUpdated = accounts.every((acc) => acc.lastScrapeAttempt === 0);
	...
	const beyond90days = olderScanDone || (user && user['custom:scan_older_done'] == '1');
					...
					<div className={`mt-4 w-840 bottom ${isTablet ? 'col' : 'col-sm-9'}`}>
						...
						{!loading && !showScanning && (
							<>
								...
								{!isEmpty(search) && (
									<h3 className="sofia-pro mt-0 ml-3 text-18 text-list">Search Results</h3>
								)}
								{isEmpty(search) && (
									<>
										<div>
											...
											<>
												<div className="row justify-center mt-2 mobile-footer-row">
													<div className="col-sm-6 text-center">
														...
														<button className="btn btn-footer" onClick={scanOlderRequest}>
					...
					{!isTablet && (
						<>
							<div className="col-sm-3 checkout-card">
								<RightCard setSelectedProducts={setSelectedProducts} beyond90days={beyond90days} />
							</div>
						</>
					)}
				</div>
