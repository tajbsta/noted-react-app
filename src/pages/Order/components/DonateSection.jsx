...
const DonateSection = (props) => {
  const { items, confirmed, onCartRemove, handleSelectDonationOrg } = props;
  ...
  useEffect(() => {
    const currentOrg = get(donationFormik.values, 'donationOrg', '');
    if (!currentOrg && items.length > 0) {
      donationFormik.validateField('donationOrg');
    }
  }, [items]);
