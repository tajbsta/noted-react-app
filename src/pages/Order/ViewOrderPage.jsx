...
import ViewOrderDonateSection from './components/ViewOrderDonateSection';
...
const ViewOrder = () => {
  ...
  useEffect(() => {
    if (order) {
      ...
      const donationOrgFromOrder = get(order, 'donationOrg', {});
      //COMPARE DONATION ORGANISATIONS
      const hasChangedDonationOrganisation = !isEqual(
        selectedDonationOrg,
        donationOrgFromOrder
      );
      ...
      hasChanges =
        ...
        hasChangedDonationOrganisation;

      setHasModifications(hasChanges);
    }
  }, [address, payment, details, items, selectedDonationOrg]);
          ...
          <div className={isMobile ? 'col-sm-12' : 'col-sm-9'}>
            ...
            {confirmed || cancelled ? (
              ...
              <div className='mobile-checkout-col'>
                {order && <ViewOrderPickUpDetails order={order} />}
              </div>
            )}
            ...
            <div className='col desktop-col'>
              ...
              {itemsToReturn.map((product, index) => {
                return (
                  <ProductCard
                    scannedItem={product}
                    key={index}
